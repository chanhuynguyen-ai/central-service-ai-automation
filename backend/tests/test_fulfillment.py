...
def test_service_agent_can_claim_and_complete_state_machine(db_session):
    ...
    item, request = act(
        ...
        agent,
        item.id,
        WorkItemAction(action="assign", version=item.version),
    )
    assert item.assignee_user_id == agent.id
    ...
    item, request = act(
        ...
        agent,
        item.id,
        WorkItemAction(action="start", version=item.version),
    )
    ...
    item, request = act(
        ...
        agent,
        item.id,
        WorkItemAction(action="wait", version=item.version),
    )
    ...
    item, request = act(
        ...
        agent,
        item.id,
        WorkItemAction(action="resume", version=item.version),
    )
    item, request = act(
        db,
        agent,
        ...
        WorkItemAction(
            ...
            note="Device replaced",
        ),
    )
    assert request.status == "resolved"
    assert item.resolution_summary == "Device replaced"

    item, request = act(
        db,
        agent,
        item.id,
        WorkItemAction(action="close", version=item.version),
    )
...
def test_unassigned_agent_cannot_operate_other_agents_work(db_session):
    db = db_session
    request = _approved_request(db)
    lead = _user(db, "service.lead@centralops.demo")
    agent = _user(db, "service.agent@centralops.demo")
...
def test_stale_version_conflicts(db_session):
    ...
    item, _ = act(
        ...
        agent,
        item.id,
        WorkItemAction(action="assign", version=1),
    )
    ...
    with pytest.raises(FulfillmentError) as error:
        act(
            ...
            agent,
            item.id,
            WorkItemAction(action="start", version=1),
        )
