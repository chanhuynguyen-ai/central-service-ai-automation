...
def intake_from_power_apps(
    payload: PowerPlatformIntake,
...
def request_status(reference: str, db: Session = Depends(get_db)) -> ServiceRequest:
    request = (
        ...
        .filter(ServiceRequest.reference == reference)
        .first()
    )
    if not request:
...
def approval_decision(
    ...
    add_audit_event(
        db,
        "power_automate_approval_decided",
        request=request,
        actor=approver,
...
@router.get(
    "/analytics-feed",
    response_model=list[AnalyticsFeedRow],
    dependencies=[Depends(verify_integration_key)],
)
def analytics_feed(
    ...
    rows = db.query(ServiceRequest).order_by(ServiceRequest.submitted_at.desc()).limit(limit).all()
    return [
        AnalyticsFeedRow(
